  ...
  return (
    <main className="min-h-screen bg-[rgb(255,250,246)] p-6 md:p-10 font-sans relative">
      ...
      {data.user && <AuthUserSync user={data.user} />}
       ...
       {/* <div className="h-[1px] -top-5  -bottom-5 bg-gradient-to-r from-transparent via-[#003366]/20 to-transparent"></div> */}
      {/* ================= HEADER ================= */}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-10 relative">
          ...
          <div className="bg-[rgb(225,220,213)] rounded-2xl p-7 shadow-xl border border-gray-200">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              {data.subjects?.slice(0, 4).map((s) => (
                <div
                  key={s.subject_id}
                  className="relative bg-gradient-to-br from-white to-gray-50 border border-gray-200 rounded-xl p-5 h-[140px] group hover:shadow-lg hover:border-[#003366]/30 transition-all duration-300 cursor-pointer overflow-hidden"
                >
                  <a
                    href={`/notes?subjectId=${s.subject_id}`}
                    className="font-bold text-lg text-gray-800 group-hover:text-[#003366] transition-colors duration-300 relative z-10 block h-full"
                  >
              ...
              {Array.from({ length: Math.max(0, 4 - (data.subjects?.length || 0)) }).map((_, i) => (
                <a
                  ...
                  className="border-2 border-dashed border-gray-300 rounded-xl h-[140px] flex flex-col items-center justify-center font-medium text-gray-400 bg-gray-50 hover:bg-gradient-to-br hover:from-gray-50 hover:to-[#003366]/5 hover:border-[#003366]/30 hover:text-[#003366] transition-all duration-300 group"
                >
        ...
        {/* ================= RIGHT SIDE TOOLS PANEL WITH VIDEO ================= */}
